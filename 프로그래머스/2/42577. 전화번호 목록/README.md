...
### 풀이 시행착오
1. 정렬 후 단순 문자열끼리 비교 -> 시간초과
2. unordered_set을 사용하여 문자열 비교 -> 테스트케이스 미통과 (3개)
3. '정렬 후' unordered_set 기반 문자열 비교 -> 정답

### 배운점
- 정렬의 필요성: 굳이 정렬이 필요한지 이해하지 못했으나, 길이가 더 긴 값이 먼저 전달되는 경우 문제가 발생할 수 있음
        => 탐색문제에서 앞의 값의 일부가 포함되었는지 확인하기 위해서는 크기순 정렬이 반드시 필요! (큰 값이 먼저 들어가버리면 탐색이 안됨)
