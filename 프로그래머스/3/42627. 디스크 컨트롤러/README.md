...
## 코드 설계과정
1. 문제에서 '정답으로 얻고자 하는 값' & '연산에 필요로 하는 정보'를 정리
   a. 정답으로 얻고자 하는 값: 각 작업 별 평균소요시간(내림)
   b. 연산에 필요로 하는 정보: 작업의 실행 소요시간 (를 구하기 위한 방법을 재귀적으로 이어가며 구상)
2. 각 과정을 효율적으로 처리하는 방법 고민
   a. 시간을 1씩 무한하게 늘리며 순회 -> 특정 시간만 순회하는 방식으로 구현 (priority_set 사용)
   b. 작업정보를 한번에 넣고 우선순위에서 시간이 바뀔 때마다 재정렬 -> 특정 시간으로 이동했을 때 시간 순으로 정렬된 작업목록에서 1개씩 빼며 추가

## 풀이 시행착오
1. 테스트케이스 지속 실패: 구현 간 방문하는 시간연산로직 오류 -> 재설계
2. 순회시간 고려로직 지속오류 -> 순회시간 연산에 priority_queue 적용
3. 사용자 지정 정렬조건 설정문제 -> 정렬개념 복습 후 설계

## 배운점
- 풀이 전 구현설계의 중요성: 머리 속으로만 생각하지 말고 손으로 직접 그려보며 설계할 것
- 효율적 풀이보다는 정확한 풀이: 효율적으로 생각하려다 예외사항을 놓치지 않도록 유념할 것
- 상황에 따른 자료구조의 효율적인 선택: 문제의 상황을 잘 고려하여 적절한 자료구조를 선택/사용할 것
